let is_turn_white;
let turn_phase; // 1=piece selection | 2=destination selection
let board = [];
let square_dests = [];
let square_orig;
let w_pawn_direction = 'asc';
let b_pawn_direction = 'desc';
...
/*
    handle keyUp on body
*/
function bodyKeyUp(target) {
    console.log(target);
}

/*
    resolve square that a pawn can be moved
*/
function movesPawn(r, c, pawnDirection) {
    let res = [];
    let auxR;
    let auxC;
    let direction = pawnDirection == 'desc' ? 1 : -1;

    if (pawnDirection == 'desc') {

        auxR = r + 1;
        auxC = 0;
        if (isEmptySquare(auxR, auxC))
            res.push([[1, 0]]);

        if (r == 1 && isEmptySquare(r + 2, c)) // first movment
            res[0].push([2, 0]);
...
/*
    Clear the destination if mouse leave board
    and turn_phase == 1
*/
function onLeaveBoard(){
    if (turn_phase == 1 )
        clearSquareDestination();
}

/* 
   Update the match message
*/
function setMatchMsg(msg) {
    let color = is_turn_white ? "White" : "Black";

    document.getElementById("msgPlayer").innerHTML = `${color}, ${msg}`; 
}

/*
  Move a piece
*/
function movePiece(rowD, colD) {
    let rowO = Number(square_orig[0]);
    let colO = Number(square_orig[1]);

    board[rowD][colD] = board[rowO][colO];
    
    board[rowO][colO] = null;
...
/* 
   Clear all squares marked as destination of a piece
*/
function clearSquareDestination(){
    square_dests.map(square => document.getElementById( "d".concat(square )).classList.remove("b-square-destination"));
    square_dests = [];
}
...
*/
function chooseDest(target) {
    let row;
    let col;
    let rowD;
    let colD;
    let positions;
    let pawnDirection = is_turn_white ? w_pawn_direction : b_pawn_direction;

    row = Number(target.children[1].id[0]);
    col = Number(target.children[1].id[1]);
    ...
    if (is_turn_white != isPieceWhite(row, col)) {
        return
    }

    square_orig = "" + row + col;

    if (target.children[1].src.includes("pawn"))
        positions = movesPawn(row, col, pawnDirection)
    else if (target.children[1].src.includes("knight"))
        positions = moves_knight
    else if (target.children[1].src.includes("bishop"))
        positions = moves_bishop;
    else if (target.children[1].src.includes("hook"))
        positions = moves_hook;
    else if (target.children[1].src.includes("queen"))
        ...
        for (j = 0; j < positions[i].length; j++) {
            ...
            if (isPieceInBoard(rowD, colD)) {
                ...
                if (isEmptySquare(rowD, colD)) {
                    document.getElementById("d" + rowD + colD).classList.add("b-square-destination");
                    square_dests.push("" + rowD + colD);
                } else {
...
/*
    this function will handle movements
*/
function handleMove(target) {
    let row;
    let col;
    let auxId = target.children[1].id;

    if (auxId[0] == "d") {
        row = Number(target.children[1].id[1]);
        col = Number(target.children[1].id[2]);
    } else {
        row = Number(target.children[1].id[0]);
        col = Number(target.children[1].id[1]);        
    }
    ...
    if (turn_phase == 1) { // choosing piece phase

        if (isEmptySquare(row, col) || isPieceWhite(row, col) != is_turn_white) 
            return
    
        setMatchMsg("choose a destination ( ESC cancel )"); 

        turn_phase = 2;            
    } else {
        
        if ( square_dests.indexOf("" + row + col) < 0) 
            return

        movePiece(row, col);
        is_turn_white = !is_turn_white;      
        turn_phase = 1;
        setMatchMsg("choose a piece"); 
...
function newGame() {
    ...
    if (Math.random() < .5) {
        ...
        b_pawn_direction = 'asc';
        w_pawn_direction = 'desc';

    } else {
        updateBoard([[...b_piece], [...b_pawn], [...n_line], [...n_line], [...n_line], [...n_line], [...w_pawn], [...w_piece]]);
        w_pawn_direction = 'asc';
        b_pawn_direction = 'desc';
    }
    is_turn_white = true;

    setMatchMsg("choose a piece to move"); 
}
...
function repeatPieceChossing(key){ 
    turn_phase = 1;
    clearSquareDestination();
    setMatchMsg("choose a piece to move"); 
}
